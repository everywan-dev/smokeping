...
# Try loading SmokePing library (Docker path)
use lib '/usr/lib/smokeping';
eval {
    require Smokeping;
    1;
} or do {
    # If unavailable, use alternative method
...
while (1) {
    ...
    print strftime("%Y-%m-%d %H:%M:%S", localtime) . " - Cycle complete. Waiting $interval seconds...\n";
    sleep($interval);
}
...
sub load_targets {
    ...
    if (defined &Smokeping::load_cfg) {
        ...
        # Alternative method: parse config file directly
        if (open(my $fh, '<', $config_file)) {
            ...
            while (my $line = <$fh>) {
                ...
                if ($line =~ /^\+(\w+)/) {
                    ...
                    my $third = $1;
                    # Look for host in following lines
                    my $host = '';
...
sub run_traceroute {
    ...
    if ($exit_code == 124 || $exit_code == 137) {
        ...
        $result = ""; # Don't store corrupted partial result
    }
    ...
    if ($dbh) {
        ...
        # 3. Detect changes (if Telegram is configured and there was a previous route)
        if ($ENV{'TELEGRAM_BOT_TOKEN'} && $last_tracert && $last_tracert ne $result) {
            # Strip timestamps/times from output to avoid false positives from jitter
...
# Helper to extract only hops (IPs) and ignore timing
sub extract_hops {
    my $trace = shift;
    my @hops;
